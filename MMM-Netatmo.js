 ...
 * Originally by By Christopher Fenner http://github.com/CFenner
 * MIT Licensed.
 */
...
Module.register('MMM-Netatmo', {
    ...
    defaults: {
        refreshToken: null,
        updateInterval: 10, // every 3 minutes, refresh interval on netatmo is 10 minutes
        animationSpeed: 1000,
    ...
    getScripts: function () {
        return [
            'String.format.js',
            '//cdnjs.cloudflare.com/ajax/libs/jquery/3.1.1/jquery.js',
            'q.min.js'
        ];
    },
    // init method
    ...
    updateWait: function () {
        ...
        const loader = $('.netatmo .loadTimer .loader');
        if (loader.length > 0) {
            loader.attr('d', anim);
        }
        const border = $('.netatmo .loadTimer .border');
        if (border.length > 0) {
            border.attr('d', anim);
        }
        if (r === 0) {
    ...
    getDom: function () {
        const dom = $('<div/>').addClass('netatmo');
        if (this.dom) {
            dom.append(
                this.dom
            ).append(
